      ...
      <div class="modal-content">
        <VForm class="form" novalidate>
          <div class="modal-header">
            <h3 class="modal-title">Export Suppliers Data</h3>
...
import { ExportSupplierData } from "@/stores/api";

export default defineComponent({
  name: "supplier-export-modal",
  components: {
    ErrorMessage,
    Field,
    VForm,
  ...
  setup(props, { emit }) {
    ...
    const onsubmit = async (exportType: "pdf" | "excel") => {
      ...
      try {
        ...
        // Make the API call using the custom ExportSupplierData function
        const response = await ExportSupplierData(data); // This will await the response from the server
        ...
        const fileName = `suppliers_${moment().format(
          "YYYYMMDD_HHmmss"
        )}.${fileType}`;
