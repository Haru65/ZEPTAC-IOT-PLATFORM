<template>
    <div
      class="modal fade"
      tabindex="-1"
      id="kt_audit_scheudle_export_modal"
        ...
        <div class="modal-content">
          <VForm class="form" novalidate>
            <div class="modal-header">
              <h3 class="modal-title">Export Internal Audits Data</h3>
  ...
  import { ExportAuditScheduleData } from "@/stores/api";
  
  export default defineComponent({
    name: "audit-schedule-export-modal",
    components: {
      ErrorMessage,
      Field,
      VForm,
    ...
    setup(props, { emit }) {
      ...
      const onsubmit = async (exportType: "pdf" | "excel") => {
        ...
        try {
          ...
          // Make the API call using the custom ExportAuditScheduleData function
          const response = await ExportAuditScheduleData(data); // This will await the response from the server
          ...
          const fileName = `audit_scheudles_${moment().format(
            "YYYYMMDD_HHmmss"
          )}.${fileType}`;
