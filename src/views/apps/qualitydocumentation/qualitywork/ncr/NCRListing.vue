      ...
      >
        ...
        <template v-slot:approval_status="{ row: ncr }">
          ...
          <span
            ...
            :class="{
              'badge-light-primary': ncr.approval_status == 1,
              'badge-light-danger': ncr.approval_status == 2,
              'badge-light-success': ncr.approval_status == 3,
            }"
          >
            {{ GetApprovalStatus(ncr.approval_status) }}
          </span>
        </template>
            ...
            >
              ...
              <li>
                <a
                  ...
                  data-bs-toggle="tooltip"
                  title="Delete NCR"
                  @click.prevent="deleteItem(ncr.id, false)"
                >
  ...
  setup() {
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteNonConformanceRecord(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await NonConformanceRecordSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
