  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:approval_status="{ row: risk }">
          <span
            v-if="risk.approval_status == 1"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >{{ GetApprovalStatus(risk.approval_status) }}</span
          >
          <span
            v-if="risk.approval_status == 2"
            class="badge py-3 px-4 fs-7 badge-light-danger"
            >{{ GetApprovalStatus(risk.approval_status) }}</span
          >
          <span
            v-if="risk.approval_status == 3"
            class="badge py-3 px-4 fs-7 badge-light-success"
            >{{ GetApprovalStatus(risk.approval_status) }}</span
          >
        </template>

        <template v-slot:approval_button="{ row: risk }">
          <button
            type="button"
            class="btn btn-sm btn-primary"
            data-bs-toggle="modal"
            data-bs-target="#kt_modal_1"
            @click="fillItemData(risk)"
          >
            Open
          </button>
        </template>
...
import { ApprovalStatus, GetApprovalStatus } from "@/core/model/risk";
  ...
  setup() {
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteRiskRegister(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await RiskRegisterSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
