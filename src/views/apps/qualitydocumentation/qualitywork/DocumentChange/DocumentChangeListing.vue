  ...
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        >
          ...
          <!--begin::Add customer-->
          <router-link
            to="/document_change_request/add"
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:approval_status="{ row: document_change }">
          <span
            v-if="document_change.approval_status == 1"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >{{ GetApprovalStatus(document_change.approval_status) }}</span
          >
          <span
            v-if="document_change.approval_status == 2"
            class="badge py-3 px-4 fs-7 badge-light-danger"
            >{{ GetApprovalStatus(document_change.approval_status) }}</span
          >
          <span
            v-if="document_change.approval_status == 3"
            class="badge py-3 px-4 fs-7 badge-light-success"
            >{{ GetApprovalStatus(document_change.approval_status) }}</span
          >
        </template>

        <template v-slot:approval_button="{ row: document_change }">
          <button
            type="button"
            class="btn btn-sm btn-primary"
            data-bs-toggle="modal"
            data-bs-target="#kt_modal_1"
            @click="fillItemData(document_change)"
          >
            Open
          </button>
        </template>
            ...
            >
              ...
              <li>
                <a
                  ...
                  @click.prevent="deleteItem(document_change.id, false)"
                >
  ...
  setup() {
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteDocumentChange(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await DocumentChangeSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
