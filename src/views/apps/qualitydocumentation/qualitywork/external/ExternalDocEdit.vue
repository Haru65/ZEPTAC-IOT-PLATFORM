              ...
              >
                ...
                <div class="mb-3 mb-md-0 fw-semobold">
                  <h4 class="text-gray-800 fw-bold cursor-pointer">
                    <a
                      target="blank"
                      v-bind:href="`https://api.zeptac.com/storage/company/${documentDetails.company_id}/external_documents/${documentDetails.document_file}`"
                      data-toggle="tooltip"
                      title="preview file"
                      class="underline"
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        let response = await getExternalDoc(itemId.toString());
    ...
    const submit = async () => {
      ...
      try {
        ...
        // Call your API here
        const response = await updateExternalDoc(itemId, documentDetails.value);
        ...
        if (response?.success) {
          ...
          router.push({ name: "external-documents" });
        } else {
