  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:area="{ row: improvement }">
          {{ GetAreaOfImprovement(improvement.area) }}
        </template>
        <template v-slot:target="{ row: improvement }">
          {{ improvement.target }}
        </template>
        <template v-slot:outcome="{ row: improvement }">
          {{ improvement.outcome }}
        </template>
        <template v-slot:quarter="{ row: improvement }">
          {{ improvement.quarter }}
        </template>
        <template v-slot:created_at="{ row: improvement }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            improvement.created_at
          }}</span>
        </template>
        <template v-slot:approval_status="{ row: improvement }">
          <span
            v-if="improvement.approval_status == 1"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >{{ GetApprovalStatus(improvement.approval_status) }}</span
          >
          <span
            v-if="improvement.approval_status == 2"
            class="badge py-3 px-4 fs-7 badge-light-danger"
            >{{ GetApprovalStatus(improvement.approval_status) }}</span
          >
          <span
            v-if="improvement.approval_status == 3"
            class="badge py-3 px-4 fs-7 badge-light-success"
            >{{ GetApprovalStatus(improvement.approval_status) }}</span
          >
        </template>

        <template v-slot:approval_button="{ row: improvement }">
          <button
            type="button"
            class="btn btn-sm btn-primary"
            data-bs-toggle="modal"
            data-bs-target="#kt_modal_1"
            @click="fillItemData(improvement)"
          >
            Open
          </button>
        </template>
        <template v-slot:actions="{ row: improvement }">
          ...
          <div class="dropdown">
            ...
            >
              <!-- Edit / View Improvement -->
              <li>
                <router-link
                  :to="`/improvements/edit/${improvement.id}`"
              ...
              <li>
                <a
                  ...
                  @click.prevent="deleteItem(improvement.id, false)"
                >
...
import { GetAreaOfImprovement } from "@/core/model/improvement";
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableHeader = ref([
      {
        columnName: "Area of Improvement",
        columnLabel: "area",
        sortEnabled: true,
        columnWidth: 100,
      },
      {
        columnName: "Target Rating",
        columnLabel: "target",
        sortEnabled: true,
        columnWidth: 100,
      },
    ...
    const PagePointer = async (page) => {
      ...
      try {
        ...
        const response = await getImprovementPlans(
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteImprovementPlan(id);
          if (response?.success) {
