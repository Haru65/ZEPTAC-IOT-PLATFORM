...
export default defineComponent({
  ...
  setup() {
    ...
    const quotationNo = route.params.quotation_no;
    ...
    onMounted(async () => {
      itemDetails.value.quotation_id = route.params.id.toString(); // Extract `id` from route params
      itemDetails.value.quotation_no = route.params.quotation_no.toString(); // Extract `quotation_no` from route params
    });
    ...
    const submit = async () => {
      ...
      try {
        ...
        const response = await addPurchaseOrder(itemDetails.value);
        if (response?.success) {
          showSuccessAlert(
            "Success",
            response.message || "Purchase Order has been successfully created!"
          );
          loading.value = false;
