  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:id="{ row: skillmatrix }">
          {{ skillmatrix.id }}
        </template>
        <template v-slot:user="{ row: skillmatrix }">
          {{ skillmatrix.user.first_name }} {{ skillmatrix.user.last_name }}
        </template>

        <template
          v-for="index in 10"
          :key="index"
          v-slot:[`skill_${index}`]="{ row: skillmatrix }"
        >
          <div v-if="skillmatrix.skills && index <= skillmatrix.skills.length">
            {{ skillmatrix.skills[index - 1].score }}
          </div>
        </template>
        ...
        <template v-slot:approval_status="{ row: skillmatrix }">
          ...
          <span
            ...
            :class="{
              'badge-light-primary': skillmatrix.approval_status == 1,
              'badge-light-danger': skillmatrix.approval_status == 2,
              'badge-light-success': skillmatrix.approval_status == 3,
            }"
          >
            {{ GetApprovalStatus(skillmatrix.approval_status) }}
          </span>
        </template>
        ...
        <template v-slot:actions="{ row: skillmatrix }">
          ...
          <div class="dropdown">
            ...
            >
              ...
              <!-- Edit / View Skill Matrix -->
              <li>
                <router-link
                  :to="`/skill_matrix/edit/${skillmatrix.id}`"
              ...
              <li>
                <a
                  ...
                  @click.prevent="deleteItem(skillmatrix.id, false)"
                >
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableData = ref<Array<ISkillMatrix>>([]);
    ...
    const searchItems = async () => {
      ...
      if (search.value.length != 0) {
        let results: Array<ISkillMatrix> = [];
