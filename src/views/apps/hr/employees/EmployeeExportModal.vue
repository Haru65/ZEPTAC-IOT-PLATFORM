      ...
      <div class="modal-content">
        <VForm class="form" novalidate>
          <div class="modal-header">
            <h3 class="modal-title">Export Employees Data</h3>
...
import { ExportEmployeeData } from "@/stores/api";

export default defineComponent({
  name: "employee-export-modal",
  components: {
    ErrorMessage,
    Field,
    VForm,
  ...
  setup(props, { emit }) {
    ...
    const onsubmit = async (exportType: "pdf" | "excel") => {
      ...
      try {
        ...
        // Make the API call using the custom ExportEmployeeData function
        const response = await ExportEmployeeData(data); // This will await the response from the server
        ...
        const fileName = `employees_${moment().format(
          "YYYYMMDD_HHmmss"
        )}.${fileType}`;
