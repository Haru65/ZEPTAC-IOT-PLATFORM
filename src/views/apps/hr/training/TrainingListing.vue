  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:id="{ row: training }">
          {{ training.id }}
        </template>
        <template v-slot:training_date="{ row: training }">
          {{ training.training_date }}
        </template>
        <template v-slot:training_topic="{ row: training }">
        ...
        <template v-slot:training_mode="{ row: training }">
          <span
            v-if="training.training_mode == 1"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >{{ GetTrainingMode(training.training_mode) }}</span
          >
          <span
            v-if="training.training_mode == 2"
            class="badge py-3 px-4 fs-7 badge-light-success"
            >{{ GetTrainingMode(training.training_mode) }}</span
          >
        </template>
        <template v-slot:training_status="{ row: training }">
          <span
            v-if="training.training_status == 1"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >{{ GetTrainingStatus(training.training_status) }}</span
          >
          <span
            v-if="training.training_status == 2"
            class="badge py-3 px-4 fs-7 badge-light-danger"
            >{{ GetTrainingStatus(training.training_status) }}</span
          >
          <span
            v-if="training.training_status == 3"
            class="badge py-3 px-4 fs-7 badge-light-success"
            >{{ GetTrainingStatus(training.training_status) }}</span
          >
        </template>
            ...
            >
              ...
              <li>
                <a
                  ...
                  @click.prevent="deleteItem(training.id, false)"
                >
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableHeader = ref([
      {
        columnName: "Training Date",
        columnLabel: "training_date",
        sortEnabled: true,
        columnWidth: 80,
      },
      {
        columnName: "Training Topic",
        columnLabel: "training_topic",
        sortEnabled: true,
        columnWidth: 175,
      },
      {
        columnName: "Trainer's Name",
        columnLabel: "trainers",
        sortEnabled: true,
        columnWidth: 175,
      },
      {
        columnName: "Training Mode",
        columnLabel: "training_mode",
        sortEnabled: true,
        columnWidth: 85,
      },
      {
        columnName: "Training Status",
        columnLabel: "training_status",
        sortEnabled: true,
        columnWidth: 85,
      },
      {
    ...
    const tableData = ref<Array<ITraining>>([]);

    const initvalues = ref<Array<ITraining>>([]);
    ...
    async function SearchMore() {
      ...
      try {
        const response = await TrainingSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
