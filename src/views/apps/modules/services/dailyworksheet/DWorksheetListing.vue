        ...
        >
          ...
          <template v-slot:approval_status="{ row: dailyworksheets }">
            ...
            <span
              ...
              :class="{
                'badge-light-primary': dailyworksheets.approval_status == 1,
                'badge-light-danger': dailyworksheets.approval_status == 2,
                'badge-light-success': dailyworksheets.approval_status == 3,
              }"
            >
              {{ GetApprovalStatus(dailyworksheets.approval_status) }}
            </span>
          </template>
              ...
              >
                ...
                <li>
                  <a
                    ...
                    @click.prevent="downloadWorksheet(dailyworksheets.id)"
                  >
                    <KTIcon
                      icon-name="file-down"
                      icon-class="fs-3 text-primary"
                ...
                <li>
                  <a
                    ...
                    @click.prevent="deleteItem(dailyworksheets.id, false)"
                  >
  ...
  setup() {
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteDailyWorksheet(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await WorksheetSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
