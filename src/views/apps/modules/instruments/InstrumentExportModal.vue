<template>
        ...
        <div class="modal-content">
          <VForm class="form" novalidate>
            <div class="modal-header">
              <h3 class="modal-title">Export Instruments Data</h3>
  ...
  import { ExportInstrumentData } from "@/stores/api";
  
  export default defineComponent({
    name: "instrument-export-modal",
    components: {
      ErrorMessage,
      Field,
      VForm,
    ...
    setup(props, { emit }) {
      ...
      const onsubmit = async (exportType: "pdf" | "excel") => {
        ...
        try {
          ...
          // Make the API call using the custom ExportInstrumentData function
          const response = await ExportInstrumentData(data); // This will await the response from the server
          ...
          const fileName = `instruments_${moment().format(
            "YYYYMMDD_HHmmss"
          )}.${fileType}`;
