...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        let response = await getQualityProcedure(itemId.toString());
        ...
        if (response.success) {
          ...
          console.error(
            `Error Occured in getQualityProcedure : ${
              response.message || "Error Occured in API"
            }`
          );
        }
      } catch (err) {
        console.error(`Error Occured in getQualityProcedure : ${err}`);
      }
    });
    ...
    const submit = async () => {
      ...
      try {
        ...
        if (result) {
          const response = await updateQualityProcedure(
            itemId,
            procedureDetails.value
          );
          if (response.success) {
