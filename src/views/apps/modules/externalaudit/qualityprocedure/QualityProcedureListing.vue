        ...
        <template v-slot:actions="{ row: qualprocedure }">
          ...
          <div class="dropdown">
            ...
            >
              ...
              <!-- Delete Quality Procedure -->
              <li>
                <a
                  class="dropdown-item d-flex align-items-center gap-3 px-4 py-3 hover-bg-light-danger cursor-pointer"
                  data-bs-toggle="tooltip"
                  title="Delete Quality Procedure"
                  @click.prevent="deleteItem(qualprocedure.id, false)"
                >
  ...
  setup() {
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteQualityProcedure(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await QualityProcedureSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
