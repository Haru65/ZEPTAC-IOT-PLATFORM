  ...
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        >
          ...
          <!--end::Export-->
          <!--begin::Add customer-->
          <router-link to="/validation/add" class="btn btn-primary">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:id="{ row: valprocedure }">
          {{ valprocedure.id }}
        </template>
        <template v-slot:document_name="{ row: valprocedure }">
          {{ valprocedure.document_name }}
        </template>
        <template v-slot:issue_date="{ row: valprocedure }">
          {{ valprocedure.issue_date }}
        </template>
        <template v-slot:issue_no="{ row: valprocedure }">
          {{ valprocedure.issue_no }}
        </template>
        <template v-slot:revision_date="{ row: valprocedure }">
          {{ valprocedure.revision_date }}
        </template>
        <template v-slot:revision_no="{ row: valprocedure }">
          {{ valprocedure.revision_no }}
        </template>
        <template v-slot:prepared_by="{ row: valprocedure }">
          {{ valprocedure.prepared_by }}
        </template>
        <template v-slot:approved_by="{ row: valprocedure }">
          {{ valprocedure.approved_by }}
        </template>
        ...
        <template v-slot:actions="{ row: valprocedure }">
          ...
          <div class="dropdown">
            ...
            >
              ...
              <!-- Delete Validation Procedure -->
              <li>
                <a
                  class="dropdown-item d-flex align-items-center gap-3 px-4 py-3 hover-bg-light-danger cursor-pointer"
                  data-bs-toggle="tooltip"
                  title="Delete Validation Procedure"
                  @click.prevent="deleteItem(valprocedure.id, false)"
                >
...
import type { IValProcedure } from "@/core/model/valprocedure";
import {
  getValidationProcedures,
  deleteValidationProcedure,
  ValidationProcedureSearch,
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableData = ref<Array<IValProcedure>>([]);

    const initvalues = ref<Array<IValProcedure>>([]);
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteValidationProcedure(id);
          if (response?.success) {
    ...
    const searchItems = async () => {
      ...
      if (search.value.length != 0) {
        let results: Array<IValProcedure> = [];
    ...
    async function SearchMore() {
      ...
      try {
        const response = await ValidationProcedureSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
