...
import { getLAFReport, updateLAFReport } from "@/stores/api";
    ...
    onMounted(async () => {
      try {
        ...
        if (response.success) {
          ...
          console.error(
            `Error Occured in getLAFReport : ${
              response.message || "Error Occured in API"
            }`
          );
        }
      } catch (err) {
        console.error(`Error Occured in getLAFReport : ${err}`);
      }
    });
      ...
      try {
        if (validateForm(itemDetails.value)) {
          ...
          if (response.success) {
            showSuccessAlert(
              "Success",
              response.message || "LAF Report has been successfully updated!"
            );
            loading.value = false;
