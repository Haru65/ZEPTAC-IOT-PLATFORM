          ...
          >
            ...
            <div class="card-body border-top p-sd-2 p-lg-9">
              ...
              <div class="tab-content" id="myTabContent">
                ...
                >
                  ...
                  <div class="mb-6 container">
                    <h2 class="accordion-header mb-6">
                      Inflow Air Velocity Measurement in m/sec
                    </h2>
                ...
                >
                  <h2 class="accordion-header mb-6">
                    DownFlow Air Velocity Measurement in m/sec
                  </h2>
                  <div class="mb-6 container">
                    <div class="alert alert-success">
                      "Acceptance limit (0.25 - 0.50 m/s)"
                    </div>
                  </div>
                  <div class="table-responsive py-6 mb-6">
                    <table
                ...
                >
                  <div class="mb-6 container">
                    <h2 class="accordion-header mb-6">
                      HEPA Filter Leakge Measurement in %
                    </h2>
                    <div class="alert alert-success">
                ...
                >
                  <div class="mb-6 container">
                    <h2 class="accordion-header mb-6">
                      Particle Concentration / Cu.Meter (m³)
                    </h2>
                    <div class="alert alert-success">
                        ...
                        >
                          ...
                          <td>
                            <input
                              ...
                              @keyup="SetParticleLs($event, index, 'acceptance_limit')"
                            />
                          </td>
                          <td>
                            <div
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const itemDetailsValidator = Yup.object().shape({
        d_receipt: Yup.string().required().label("Audit Name"),
        d_testing: Yup.string().required().label("Audit Name"),
        d_issue: Yup.string().required().label("Audit Name"),
        r_due_date: Yup.string().required().label("Audit Name"),
  
        description: Yup.string().required().label("Audit Name"),
        make: Yup.string().required().label("Audit Name"),
        model_no: Yup.string().required().label("Audit Name"),
        serial_no: Yup.string().required().label("Audit Name"),
        id_no: Yup.string().required().label("Audit Name"),
        testing_at: Yup.string().required().label("Audit Name"),
        room_name: Yup.string().required().label("Audit Name"),
        room_id: Yup.string().required().label("Audit Name"),
  
        temperature: Yup.string().required().label("Adreess"),
        humidity: Yup.string().required().label("Adreess"),
        differential_pressure: Yup.string().required().label("Adreess"),
      });
      ...
      onMounted(async () => {
        ...
        let response = await getLAFReport(itemId.toString());
        console.log(response);
  
        if (response != null) {
          itemDetails.value = {
            id: response.id,
            laf_id: response.laf_id,
      ...
      const submit = async () => {
        ...
        try {
          if (validateForm(itemDetails.value)) {
            const response = await updateLAFReport(itemId, itemDetails.value);
            if (!response.error) {
              showSuccessAlert(
                "Success",
