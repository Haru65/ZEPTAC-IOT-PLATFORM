  ...
  setup() {
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteLAFReport(id);
          if (response?.success) {
    ...
    */
    ...
    const downloadPdf = async (id: any) => {
      try {
        ...
        const response = await DownloadLaminarAirFlow(data); // This will await the response from the server
        ...
        const fileType = "pdf";
        const fileName = `lamiar_air_flow_report_${moment().format(
          "YYYYMMDD_HHmmss"
        )}.${fileType}`;
