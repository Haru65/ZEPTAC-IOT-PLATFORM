<template>
    <div
      class="modal fade"
      tabindex="-1"
      id="kt_thermal_instrument_export_modal"
        ...
        <div class="modal-content">
          <VForm class="form" novalidate>
            <div class="modal-header">
              <h3 class="modal-title">Export Thermal Instruments Data</h3>
  ...
  import { ExportThermalInstrumentData } from "@/stores/api";
  
  export default defineComponent({
    name: "thermal-instrument-export-modal",
    components: {
      ErrorMessage,
      Field,
      VForm,
    ...
    setup(props, { emit }) {
      ...
      const onsubmit = async (exportType: "pdf" | "excel") => {
        ...
        try {
          ...
          // Make the API call using the custom ExportThermalInstrumentData function
          const response = await ExportThermalInstrumentData(data); // This will await the response from the server
          ...
          const fileName = `thermal_instruments_${moment().format(
            "YYYYMMDD_HHmmss"
          )}.${fileType}`;
