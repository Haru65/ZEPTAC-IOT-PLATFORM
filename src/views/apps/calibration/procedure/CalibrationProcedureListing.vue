  ...
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        >
          ...
          <!--end::Export-->
          <!--begin::Add customer-->
          <router-link to="/calibration-procedure/add" class="btn btn-primary">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:id="{ row: calprocedure }">
          {{ calprocedure.id }}
        </template>
        <template v-slot:document_name="{ row: calprocedure }">
          {{ calprocedure.document_name }}
        </template>
        <template v-slot:issue_date="{ row: calprocedure }">
          {{ calprocedure.issue_date }}
        </template>
        <template v-slot:issue_no="{ row: calprocedure }">
          {{ calprocedure.issue_no }}
        </template>
        <template v-slot:revision_date="{ row: calprocedure }">
          {{ calprocedure.revision_date }}
        </template>
        <template v-slot:revision_no="{ row: calprocedure }">
          {{ calprocedure.revision_no }}
        </template>
        <template v-slot:prepared_by="{ row: calprocedure }">
          {{ calprocedure.prepared_by }}
        </template>
        <template v-slot:approved_by="{ row: calprocedure }">
          {{ calprocedure.approved_by }}
        </template>
        <template v-slot:created_at="{ row: calprocedure }">
          {{ calprocedure.created_at }}
        </template>
        <template v-slot:audit_document="{ row: calprocedure }">
          <!--begin::Menu Flex-->
        ...
        <template v-slot:actions="{ row: calprocedure }">
          <!--begin::Menu Flex-->
          <div class="dropdown">
            <a
              href="#"
            ...
            >
              <!-- View Calibration Procedure -->
              <li>
                <router-link
                  :to="`/calibration-procedure/edit/${calprocedure.id}`"
              ...
              <li>
                <a
                  ...
                  @click.prevent="deleteItem(calprocedure.id, false)"
                >
...
import type { ICalProcedure } from "@/core/model/calprocedure";
import {
  getCalibrationProcedures,
  deleteCalibrationProcedure,
  CalibrationProcedureSearch,
} from "@/stores/api";
  ...
  setup() {
    ...
    const PagePointer = async (page) => {
      ...
      try {
        ...
        const response = await getCalibrationProcedures(
    ...
    const tableData = ref<Array<ICalProcedure>>([]);

    const initvalues = ref<Array<ICalProcedure>>([]);

    async function validation_listing(): Promise<void> {
      try {
        const response = await getCalibrationProcedures(
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteCalibrationProcedure(id);
          if (response?.success) {
    ...
    const searchItems = async () => {
      ...
      if (search.value.length != 0) {
        let results: Array<ICalProcedure> = [];
    ...
    async function SearchMore() {
      ...
      try {
        const response = await CalibrationProcedureSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
