  ...
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-title">
        ...
        <div class="d-flex align-items-center position-relative my-1">
          ...
          <input
            ...
            placeholder="Search by UUC name"
          />
        </div>
        <!--end::Search-->
      </div>
      ...
      <div class="card-toolbar">
        ...
        >
          ...
          <!--end::Export-->
          <!--begin::Add customer-->
          <router-link to="/replicate_report/add" class="btn btn-primary">
    ...
    <div class="card-body pt-0">
      ...
      <Datatable
        ...
        :loading="loading"
      >
        <template v-slot:id="{ row: replicate_report }">
          {{ replicate_report.id }}
        </template>
        <template v-slot:uuc_details="{ row: replicate_report }">
          {{ replicate_report.uuc_details.name }}
        </template>
        <template v-slot:master_equipment_1="{ row: replicate_report }">
          {{ replicate_report.master_equipment_1.name }}
        </template>
        <template v-slot:engineer_1="{ row: replicate_report }">
          {{ replicate_report.engineer_1 }}
        </template>
        <template v-slot:engineer_2="{ row: replicate_report }">
          {{ replicate_report.engineer_2 }}
        </template>
        <template v-slot:replicate_date="{ row: replicate_report }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            replicate_report.replicate_date
          }}</span>
        </template>

        <template v-slot:approval_status="{ row: replicate_report }">
          <!-- Status Badge Only -->
          <span
            class="badge py-3 px-4 fs-7"
            :class="{
              'badge-light-primary': replicate_report.approval_status == 1,
              'badge-light-danger': replicate_report.approval_status == 2,
              'badge-light-success': replicate_report.approval_status == 3,
            }"
          >
            {{ GetApprovalStatus(replicate_report.approval_status) }}
          </span>
        </template>
        ...
        <template v-slot:actions="{ row: replicate_report }">
          ...
          <div class="dropdown">
            ...
            >
              ...
              >
                <li>
                  <a
                    ...
                    @click="fillItemData(replicate_report)"
                  >
                    <KTIcon
              ...
              <!-- Edit / View Replicate Report -->
              <li>
                <router-link
                  :to="`/replicate_report/edit/${replicate_report.id}`"
              ...
              <li>
                <a
                  ...
                  @click.prevent="deleteItem(replicate_report.id, false)"
                >
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableHeader = ref([
      {
        columnName: "UUC Name",
        columnLabel: "uuc_details",
        sortEnabled: true,
        columnWidth: 125,
      },
      {
        columnName: "Master Instrument",
        columnLabel: "master_equipment_1",
        sortEnabled: true,
        columnWidth: 125,
      },
      {
        columnName: "Engineer 1",
        columnLabel: "engineer_1",
        sortEnabled: true,
        columnWidth: 100,
      },
      {
        columnName: "Engineer 2",
        columnLabel: "engineer_2",
        sortEnabled: true,
        columnWidth: 100,
      },
      {
        columnName: "Replicate Date",
        columnLabel: "replicate_date",
        sortEnabled: true,
        columnWidth: 80,
      },
      {
    ...
    const PagePointer = async (page) => {
      ...
      try {
        ...
        const response = await getReplicateReports(
    ...
    const tableData = ref<Array<IRReport>>([]);

    const initvalues = ref<Array<IRReport>>([]);
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteReplicateReport(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await ReplicateReportSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
