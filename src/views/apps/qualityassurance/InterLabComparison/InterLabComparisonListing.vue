  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:id="{ row: interlaboratory }">
          {{ interlaboratory.id }}
        </template>
        <template v-slot:lab_date="{ row: interlaboratory }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            interlaboratory.lab_date
          }}</span>
        </template>
        <template v-slot:reference_lab_name="{ row: interlaboratory }">
          {{ interlaboratory.reference_lab_name }}
        </template>

        <template v-slot:participate_lab_name="{ row: interlaboratory }">
          {{ interlaboratory.participate_lab_name }}
        </template>

        <template v-slot:artifact_name="{ row: interlaboratory }">
          {{ interlaboratory.artifact_name }}
        </template>
        <template v-slot:ranges="{ row: interlaboratory }">
          {{ interlaboratory.ranges }}
        </template>

        <template v-slot:status="{ row: interlaboratory }">
          <span
            v-if="interlaboratory.status == 1"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >En &le; 1</span
          >
          <span
            v-if="interlaboratory.status == 2"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >En &gt; 1</span
          >
        </template>
        <template v-slot:remark="{ row: interlaboratory }">
          <span
            v-if="interlaboratory.status == 1"
        ...
        <template v-slot:approval_status="{ row: interlaboratory }">
          <!-- Status Badge Only -->
          <span
            class="badge py-3 px-4 fs-7"
            :class="{
              'badge-light-primary': interlaboratory.approval_status == 1,
              'badge-light-danger': interlaboratory.approval_status == 2,
              'badge-light-success': interlaboratory.approval_status == 3,
            }"
          >
            {{ GetApprovalStatus(interlaboratory.approval_status) }}
          </span>
        </template>
        ...
        <template v-slot:actions="{ row: interlaboratory }">
          ...
          <div class="dropdown">
            ...
            >
              ...
              >
                <li>
                  <a
                    ...
                    @click="fillItemData(interlaboratory)"
                  >
                    <KTIcon
              ...
              <!-- Edit / View InterLaboratory -->
              <li>
                <router-link
                  :to="`/interlaboratory/edit/${interlaboratory.id}`"
              ...
              <li>
                <a
                  ...
                  @click.prevent="deleteItem(interlaboratory.id, false)"
                >
...
import type { ILaboratoryComparison } from "@/core/model/interlaboratory";
import {
  getInterLabComparisons,
  deleteInterLabComparison,
  InterLabComparisonSearch,
} from "@/stores/api";
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableHeader = ref([
      {
        columnName: "Date",
        columnLabel: "lab_date",
        sortEnabled: true,
        columnWidth: 100,
      },
      {
        columnName: "Reference Laboratory",
        columnLabel: "reference_lab_name",
        sortEnabled: true,
        columnWidth: 100,
      },
      {
        columnName: "Participating Laboratory",
        columnLabel: "participate_lab_name",
        sortEnabled: true,
        columnWidth: 100,
      },
      {
        columnName: "Artifact Name",
        columnLabel: "artifact_name",
        sortEnabled: true,
        columnWidth: 100,
      },
      {
    ...
    const PagePointer = async (page) => {
      ...
      try {
        ...
        const response = await getInterLabComparisons(
    ...
    const tableData = ref<Array<ILaboratoryComparison>>([]);

    const initvalues = ref<Array<ILaboratoryComparison>>([]);

    async function interlab_listing(): Promise<void> {
      try {
        const response = await getInterLabComparisons(
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteInterLabComparison(id);
          if (response?.success) {
    ...
    const searchItems = async () => {
      ...
      if (search.value.length != 0) {
        let results: Array<ILaboratoryComparison> = [];
