  ...
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        >
          ...
          <!--end::Export-->
          <!--begin::Add customer-->
          <router-link to="/po-payment/add" class="btn btn-primary">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:id="{ row: popayment }">
          {{ popayment.id }}
        </template>
        <template v-slot:po_number="{ row: popayment }">
          <span v-if="popayment.purchase_order != null">
            {{ popayment?.purchase_order.po_number || "" }}
          </span>
          <span v-else> </span>
        </template>

        <template v-slot:customer_information="{ row: popayment }">
          <span v-if="popayment?.customer_information != null">
            {{ popayment?.customer_information["company_name"] || "" }}
          </span>
          <span v-else> </span>
        </template>
        <template v-slot:payment_method="{ row: popayment }">
          {{ popayment.payment_method }}
        </template>
        <template v-slot:payment_date="{ row: popayment }">
          {{ popayment.payment_date }}
        </template>
        <template v-slot:amount_paid="{ row: popayment }">
          {{ formatPrice(popayment.amount_paid) }}
        </template>
        <template v-slot:status="{ row: popayment }">
          <!-- depending on status badge change -->
          <span
            v-if="popayment.status == 0"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >{{ GetPoPaymentStatus(popayment.status) }}</span
          >
          <span
            v-else-if="popayment.status == 1"
            class="badge py-3 px-4 fs-7 badge-light-success"
            >{{ GetPoPaymentStatus(popayment.status) }}</span
          >
          <span
            v-else-if="popayment.status == 2"
            class="badge py-3 px-4 fs-7 badge-light-danger"
            >{{ GetPoPaymentStatus(popayment.status) }}</span
          >
        </template>
        <template v-slot:actions="{ row: popayment }">
          <!--begin::Menu Flex-->
          <div class="dropdown">
            <a
              href="#"
            ...
            >
              <!-- Edit / View PO Payment -->
              <li>
                <router-link
                  :to="`/po-payment/edit/${popayment.id}`"
...
import { getPoPayments, deletePoPayment, PoPaymentSearch } from "@/stores/api";
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableHeader = ref([
      {
        columnName: "PO No.",
        columnLabel: "po_number",
        sortEnabled: true,
        columnWidth: 35,
      },
      {
        columnName: "Customer Name",
        columnLabel: "customer_information",
        sortEnabled: true,
        columnWidth: 175,
      },
      {
        columnName: "Payment Via",
    ...
    const tableData = ref<Array<IPoPayment>>([]);

    const initvalues = ref<Array<IPoPayment>>([]);
    ...
    const PagePointer = async (page) => {
      ...
      try {
        ...
        const response = await getPoPayments(
        ...
        if (response.success) {
          ...
          tableData.value = response.result.data.map(
            ({ id, purchase_order, customer_information, ...rest }) => ({
              id: id,
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deletePoPayment(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await PoPaymentSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
