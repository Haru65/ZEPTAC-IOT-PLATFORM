<template>
        ...
        <div class="modal-content">
          <VForm class="form" novalidate>
            <div class="modal-header">
              <h3 class="modal-title">Export Payments of PO</h3>
  ...
  import { ExportPoPaymentData } from "@/stores/api";
  
  export default defineComponent({
    name: "po-payment-export-modal",
    components: {
      ErrorMessage,
      Field,
      VForm,
    ...
    setup(props, { emit }) {
      ...
      const onsubmit = async (exportType: "pdf" | "excel") => {
        ...
        try {
          ...
          // Make the API call
          const response = await ExportPoPaymentData(data); // This will await the response from the server
          ...
          const fileName = `po_payments_${moment().format(
            "YYYYMMDD_HHmmss"
          )}.${fileType}`;
