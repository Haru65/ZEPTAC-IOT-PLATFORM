  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:quotation_no="{ row: po }">
          {{ po.quotation_no }}
        </template>
        ...
        <template v-slot:po_date="{ row: po }">
          {{ po.po_date }}
        </template>
        <template v-slot:total="{ row: po }">
          {{ formatPrice(po.total) }}
        </template>
        <template v-slot:balance="{ row: po }">
          {{
            po.total !== undefined
              ? formatPrice(
                  (
                    Number(po.total) -
                    (Number(po.total_paid || 0) + Number(po.total_pending || 0))
                  ).toString()
                )
              : "-"
          }}
        </template>
        <template v-slot:status="{ row: po }">
          <!-- depending on status badge change -->
          <span
            v-if="po.status == 0"
            class="badge py-3 px-4 fs-7 badge-light-danger"
            >{{ GetPoStatus(po.status) }}</span
          >
          <span
            v-if="po.status == 1"
        ...
        <template v-slot:approval_status="{ row: po }">
                    <!-- Status Badge Only -->
                    <span
            class="badge py-3 px-4 fs-7"
            :class="{
              'badge-light-primary': po.approval_status == 1,
              'badge-light-danger': po.approval_status == 2,
              'badge-light-success': po.approval_status == 3,
            }"
          >
            {{ GetApprovalStatus(po.approval_status) }}
          </span>
        </template>
          ...
          <div class="dropdown">
            ...
            <template v-if="identifier === 'Admin' || identifier === 'Company-Admin'">
              <li>
                ...
                <a
                  v-else
                  class="dropdown-item d-flex align-items-center gap-3 px-4 py-3 hover-bg-light-primary cursor-pointer disabled"
                >
                  <KTIcon
                    icon-name="check-circle"
                    icon-class="fs-3 text-muted"
                  />
...
import { PoStatus, GetPoStatus, type IPurchaseOrder } from "@/core/model/po";
import {
  getPurchaseOrders,
  deletePurchaseOrder,
  PurchaseOrderSearch,
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableData = ref<Array<IPurchaseOrder>>([]);

    const initvalues = ref<Array<IPurchaseOrder>>([]);
      ...
      try {
        ...
        if (response.success) {
          ...
          tableData.value = response.result.data.map(
            ({ id, customer, total_paid, total_pending, ...rest }) => ({
              id: id,
              customer: { ...customer },
              total_paid,
              total_pending,
    ...
    async function po_listing(): Promise<void> {
      try {
        const response = await getPurchaseOrders(
    ...
    const searchItems = async () => {
      ...
      if (search.value !== "") {
        let results: Array<IPurchaseOrder> = [];
    ...
    async function SearchMore() {
      ...
      try {
        const response = await PurchaseOrderSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
    ...
    // Function
    const fillItemData = (tabledata) => {
      const { id, approval_status, comment_description, company_id } =
        tabledata;
