      ...
      <div class="modal-content">
        <VForm class="form" novalidate>
          <div class="modal-header">
            <h3 class="modal-title">Export Invoice Data</h3>
...
import { ExportInvoiceData } from "@/stores/api";

export default defineComponent({
  name: "invoice-export-modal",
  components: {
    ErrorMessage,
    Field,
    VForm,
  ...
  setup(props, { emit }) {
    ...
    const onsubmit = async (exportType: "pdf" | "excel") => {
      ...
      try {
        ...
        // Make the API call using the custom ExportInvoiceData function
        const response = await ExportInvoiceData(data); // This will await the response from the server
        ...
        const fileName = `invoices_${moment().format(
          "YYYYMMDD_HHmmss"
        )}.${fileType}`;
