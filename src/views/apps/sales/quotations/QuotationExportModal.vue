      ...
      <div class="modal-content">
        <VForm class="form" novalidate>
          <div class="modal-header">
            <h3 class="modal-title">Export Quotation Data</h3>
...
import { ExportQuotationData } from "@/stores/api";

export default defineComponent({
  name: "quotation-export-modal",
  components: {
    ErrorMessage,
    Field,
    VForm,
  ...
  setup(props, { emit }) {
    ...
    const onsubmit = async (exportType: "pdf" | "excel") => {
      ...
      try {
        ...
        // Make the API call using the custom ExportQuotationData function
        const response = await ExportQuotationData(data); // This will await the response from the server
        ...
        const fileName = `quotations_${moment().format(
          "YYYYMMDD_HHmmss"
        )}.${fileType}`;
