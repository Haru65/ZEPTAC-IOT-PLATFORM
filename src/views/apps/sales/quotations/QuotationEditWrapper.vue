<template>
  <div>
    ...
    <component v-else :is="QuotationComponent" :id="routeId" />
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const QuotationComponent = shallowRef(null); // Prevent reactivity issues
    ...
    watchEffect(() => {
      ...
      switch (billingFormat) {
        case "billing-format-1":
          importComponent("QuotationFormat1", "QuotationFormat1Edit").then(
            (component) => (QuotationComponent.value = component)
          );
          break;
        case "billing-format-2":
          importComponent("QuotationFormat2", "QuotationFormat2Edit").then(
            (component) => (QuotationComponent.value = component)
          );
          break;
        default:
          importComponent("QuotationFormat1", "QuotationFormat1Edit").then(
            (component) => (QuotationComponent.value = component) // Fallback component
          );
      }
    });

    return { QuotationComponent, isLoading, routeId };
  },
});
</script>
