  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      <Datatable @on-sort="sort" @on-items-select="onItemSelect" :data="tableData" :header="tableHeader"
        ...
        <!-- img data -->
        <template v-slot:id="{ row: quotation }">
          <span class="text-gray-600 text-hover-primary mb-1">
            {{ quotation.id }}
          </span>
        </template>
        <template v-slot:customer_name="{ row: quotation }">
          <span class="text-gray-600 text-hover-primary mb-1">
            {{ quotation.customer_name }}
          </span>
        </template>
        <!-- defualt data -->
        <template v-slot:company="{ row: quotation }">
          {{ quotation.company }}
        </template>
        <template v-slot:date="{ row: quotation }">
          {{ quotation.date }}
        </template>
        <template v-slot:actions="{ row: quotation }">
          ...
          <div
            ...
            <!--begin::Menu item-->
            <div class="menu-item px-3">
              <a @click="deleteCustomer(quotation.id)" class="menu-link px-3">Delete</a>
            </div>
            <!--end::Menu item-->
          </div>
          <!--end::Menu-->
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableData = ref<Array<IQuotations>>(quotations);
    const initCustomers = ref<Array<IQuotations>>([]);
