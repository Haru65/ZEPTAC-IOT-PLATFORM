<template>
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        <!-- img data -->
        <template v-slot:id="{ row: nabl }">
          {{ nabl.id }}
        </template>
        <template v-slot:document_section="{ row: nabl }">
          {{ nabl.document_section }}
        </template>
        <template v-slot:document_name="{ row: nabl }">
          {{ nabl.document_name }}
        </template>
        <template v-slot:issue_date="{ row: nabl }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            nabl.issue_date
          }}</span>
        </template>
        <template v-slot:amendment_date="{ row: nabl }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            nabl.amendment_date
          }}</span>
        </template>
        <template v-slot:storage_medium="{ row: nabl }">
          {{ nabl.storage_medium }}
        </template>
        <template v-slot:document_file="{ row: nabl }">
          <!--begin::Menu Flex-->
        ...
        <template v-slot:approval_status="{ row: nabl }">
          ...
          <span
            ...
            :class="{
              'badge-light-primary': nabl.approval_status == 1,
              'badge-light-danger': nabl.approval_status == 2,
              'badge-light-success': nabl.approval_status == 3,
            }"
          >
            {{ GetApprovalStatus(nabl.approval_status) }}
          </span>
        </template>
        ...
        <template v-slot:actions="{ row: nabl }">
          ...
          <div class="dropdown">
            ...
            <template v-if="identifier === 'Admin' || identifier === 'Company-Admin'">
              ...
              <!-- Edit / View NABL Document -->
              <li>
                <router-link
                  :to="`/nabl_documents/edit/${nabl.id}`"
...
import { getNABLDocs, deleteNABLDoc, NABLDocSearch } from "@/stores/api";
import arraySort from "array-sort";
import moment from "moment";
import Swal from "sweetalert2";

import NABLDocumentAddModal from "./NABLDocumentAddModal.vue";
  ...
  setup() {
    ...
    const PagePointer = async (page) => {
      ...
      try {
        ...
        const response = await getNABLDocs(
    ...
    async function nabl_listing(): Promise<void> {
      try {
        const response = await getNABLDocs(
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteNABLDoc(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await NABLDocSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
