<template>
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        >
          ...
          <RecordsAddModal @document-added="reLoadData"></RecordsAddModal>
        </div>
        <!--end::Toolbar-->
        <!--begin::Group actions-->
        <div
    ...
    <div class="card-body pt-0">
      ...
      >
        <!-- img data -->
        <template v-slot:id="{ row: record }">
          {{ record.id }}
        </template>
        <template v-slot:document_section="{ row: record }">
          {{ record.document_section }}
        </template>
        <template v-slot:document_name="{ row: record }">
          {{ record.document_name }}
        </template>
        <template v-slot:issue_date="{ row: record }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            record.issue_date
          }}</span>
        </template>
        <template v-slot:amendment_date="{ row: record }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            record.amendment_date
          }}</span>
        </template>
        <template v-slot:storage_medium="{ row: record }">
          {{ record.storage_medium }}
        </template>
        <template v-slot:responsible_person="{ row: record }">
...
import type { IDocument } from "@/core/model/record";
import { ApprovalStatus, GetApprovalStatus } from "@/core/model/global";
import { hideModal } from "@/core/helpers/dom";
import ApprovalModal from "./ApprovalModal.vue";
import { useAuthStore } from "@/stores/auth";
import { Identifier } from "@/core/config/WhichUserConfig";
import { getRecords, deleteRecord, RecordSearch } from "@/stores/api";
import arraySort from "array-sort";
import moment from "moment";
import Swal from "sweetalert2";

import RecordsAddModal from "./RecordsAddModal.vue";
  ...
  setup() {
    ...
    const PagePointer = async (page) => {
      ...
      try {
        ...
        const response = await getRecords(`page=${page}&limit=${limit.value}&year=${
    ...
    const PageLimitPoiner = async (limit) => {
      ...
      try {
        ...
        const response = await getRecords(`page=${page.value}&limit=${limit}&year=${
            selectedYearCache.value
              ? selectedYearCache.value
              : financialYears.value[0]
          }`);
    ...
    async function SearchMore() {
      ...
      try {
        const response = await RecordSearch(search.value, selectedYearCache.value ? selectedYearCache.value : financialYears.value[0]);
