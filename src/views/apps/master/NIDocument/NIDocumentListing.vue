<template>
  <div class="card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        ...
        >
          ...
          <NIDocumentAddModal @document-added="reLoadData"></NIDocumentAddModal>
        </div>
        <!--end::Toolbar-->
        <!--begin::Group actions-->
        <div
    ...
    <div class="card-body pt-0">
      ...
      >
        <!-- img data -->
        <template v-slot:id="{ row: ni }">
          {{ ni.id }}
        </template>
        <template v-slot:document_section="{ row: ni }">
          {{ ni.document_section }}
        </template>
        <template v-slot:document_name="{ row: ni }">
          {{ ni.document_name }}
        </template>
        <template v-slot:issue_date="{ row: ni }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            ni.issue_date
          }}</span>
        </template>
        <template v-slot:amendment_date="{ row: ni }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            ni.amendment_date
          }}</span>
        </template>
        <template v-slot:storage_medium="{ row: ni }">
          {{ ni.storage_medium }}
        </template>
        <template v-slot:document_file="{ row: ni }">
          <!--begin::Menu Flex-->
          <div class="d-flex flex-lg-row">
            <a
              target="blank"
              v-bind:href="`https://api.zeptac.com/storage/company/${ni.company_id}/national_international_documents/${ni.document_file}`"
              data-toggle="tooltip"
              title="Download File"
        ...
        <template v-slot:approval_status="{ row: ni }">
          <!-- Status Badge Only -->
          <span
            class="badge py-3 px-4 fs-7"
            :class="{
              'badge-light-primary': ni.approval_status == 1,
              'badge-light-danger': ni.approval_status == 2,
              'badge-light-success': ni.approval_status == 3,
            }"
          >
            {{ GetApprovalStatus(ni.approval_status) }}
          </span>
        </template>

        <template v-slot:actions="{ row: ni }">
          <!--begin::Menu Flex-->
          <div class="dropdown">
            <a
              href="#"
            ...
            <template v-if="identifier === 'Admin' || identifier === 'Company-Admin'">
              ...
              <!-- Edit / View NI Document -->
              <li>
                <router-link
                  :to="`/ni_documents/edit/${ni.id}`"
              ...
              <li>
                <a
                  ...
                  data-bs-toggle="tooltip"
                  title="Delete NI Doc"
                  @click.prevent="deleteItem(ni.id, false)"
                >
...
import { getNIDocs, deleteNIDoc, NIDocSearch } from "@/stores/api";
import arraySort from "array-sort";
import moment from "moment";
import Swal from "sweetalert2";

import NIDocumentAddModal from "./NIDocumentAddModal.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await NIDocSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
