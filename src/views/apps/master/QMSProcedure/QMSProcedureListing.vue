<template>
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        <!-- img data -->
        <template v-slot:id="{ row: qms }">
          {{ qms.id }}
        </template>
        <template v-slot:document_section="{ row: qms }">
          {{ qms.document_section }}
        </template>
        <template v-slot:document_name="{ row: qms }">
          {{ qms.document_name }}
        </template>
        <template v-slot:issue_date="{ row: qms }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            qms.issue_date
          }}</span>
        </template>
        <template v-slot:amendment_date="{ row: qms }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            qms.amendment_date
          }}</span>
        </template>
        <template v-slot:storage_medium="{ row: qms }">
          {{ qms.storage_medium }}
        </template>
        <template v-slot:responsible_person="{ row: qms }">
          {{ qms.responsible_person }}
        </template>
        <template v-slot:document_file="{ row: qms }">
          <!--begin::Menu Flex-->
          <div class="d-flex flex-lg-row">
            <a
              target="blank"
              v-bind:href="`https://api.zeptac.com/storage/company/${qms.company_id}/qms/${qms.document_file}`"
              data-toggle="tooltip"
              title="Download File"
        ...
        <template v-slot:approval_status="{ row: qms }">
          <span
            v-if="qms.approval_status == 1"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >{{ GetApprovalStatus(qms.approval_status) }}</span
          >
          <span
            v-if="qms.approval_status == 2"
            class="badge py-3 px-4 fs-7 badge-light-danger"
            >{{ GetApprovalStatus(qms.approval_status) }}</span
          >
          <span
            v-if="qms.approval_status == 3"
            class="badge py-3 px-4 fs-7 badge-light-success"
            >{{ GetApprovalStatus(qms.approval_status) }}</span
          >
        </template>

        <template v-slot:approval_button="{ row: qms }">
          <button
            type="button"
            class="btn btn-sm btn-primary"
            data-bs-toggle="modal"
            data-bs-target="#kt_modal_1"
            @click="fillItemData(qms)"
          >
            Open
          </button>
        </template>

        <template v-slot:actions="{ row: qms }">
          <!--begin::Menu Flex-->
          <div class="dropdown">
            <a
              href="#"
...
import type { IDocument } from "@/core/model/qms";
...
import QMSProcedureAddModal from "./QMSProcedureAddModal.vue";
  ...
  setup() {
    ...
    const PagePointer = async (page) => {
      ...
      try {
        ...
        const response = await getQMSProcedures(
    ...
    const deleteItem = async (id: number, mul: boolean) => {
      ...
      const deleteFromTable = async (id: number) => {
        try {
          const response = await deleteQMSProcedure(id);
          if (response?.success) {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await QMSProcedureSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
