<template>
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        <!-- img data -->
        <template v-slot:id="{ row: workinstruction }">
          {{ workinstruction.id }}
        </template>
        <template v-slot:document_section="{ row: workinstruction }">
          {{ workinstruction.document_section }}
        </template>
        <template v-slot:document_name="{ row: workinstruction }">
          {{ workinstruction.document_name }}
        </template>
        <template v-slot:issue_date="{ row: workinstruction }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            workinstruction.issue_date
          }}</span>
        </template>
        <template v-slot:amendment_date="{ row: workinstruction }">
          <span class="badge py-3 px-4 fs-7 badge-light-primary">{{
            workinstruction.amendment_date
          }}</span>
        </template>
        <template v-slot:storage_medium="{ row: workinstruction }">
          {{ workinstruction.storage_medium }}
        </template>
        <template v-slot:responsible_person="{ row: workinstruction }">
          {{ workinstruction.responsible_person }}
        </template>
        <template v-slot:document_file="{ row: workinstruction }">
          <!--begin::Menu Flex-->
        ...
        <template v-slot:approval_status="{ row: workinstruction }">
          <span
            v-if="workinstruction.approval_status == 1"
            class="badge py-3 px-4 fs-7 badge-light-primary"
            >{{ GetApprovalStatus(workinstruction.approval_status) }}</span
          >
          <span
            v-if="workinstruction.approval_status == 2"
            class="badge py-3 px-4 fs-7 badge-light-danger"
            >{{ GetApprovalStatus(workinstruction.approval_status) }}</span
          >
          <span
            v-if="workinstruction.approval_status == 3"
            class="badge py-3 px-4 fs-7 badge-light-success"
            >{{ GetApprovalStatus(workinstruction.approval_status) }}</span
          >
        </template>

        <template v-slot:approval_button="{ row: workinstruction }">
          <button
            type="button"
            class="btn btn-sm btn-primary"
            data-bs-toggle="modal"
            data-bs-target="#kt_modal_1"
            @click="fillItemData(workinstruction)"
          >
            Open
          </button>
        </template>

        <template v-slot:actions="{ row: workinstruction }">
          <!--begin::Menu Flex-->
          <div class="dropdown">
            <a
              href="#"
            ...
            >
              <!-- Edit / View Work Instruction -->
              <li>
                <router-link
                  :to="`/work_instructions/edit/${workinstruction.id}`"
              ...
              <li>
                <a
                  ...
                  @click.prevent="deleteItem(workinstruction.id, false)"
                >
...
import {
  ...
  WorkInstructionSearch,
} from "@/stores/api";
import arraySort from "array-sort";
import moment from "moment";
import Swal from "sweetalert2";

import WorkInstructionAddModal from "./WorkInstructionAddModal.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await WorkInstructionSearch(
          search.value,
          selectedYearCache.value
            ? selectedYearCache.value
            : financialYears.value[0]
