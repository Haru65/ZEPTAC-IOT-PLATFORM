              ...
              >
                ...
                <div class="mb-3 mb-md-0 fw-semobold">
                  <h4 class="text-gray-800 fw-bold cursor-pointer">
                    <a
                      target="blank"
                      v-bind:href="`https://api.zeptac.com/storage/company/${documentDetails.company_id}/work_instructions/${documentDetails.document_file}`"
                      v-tooltip
                      title="preview file"
                      class="underline"
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        const response = await getWorkInstruction(itemId.toString());
        ...
        if (response.success) {
          ...
          documentDetails.value = {
            ...
            document_file: response.result.document_file
              ? response.result.document_file
              : "",
          ...
          console.error(
            `Error Occured in getWorkInstruction : ${
              response.message || "Error Occured in API"
            }`
          );
        }
      } catch (err) {
        console.error(`Error Occured in getWorkInstruction : ${err}`);
      }
    });
    ...
    const submit = async () => {
      ...
      try {
        ...
        // Call your API here
        const response = await updateWorkInstruction(
          itemId,
          documentDetails.value
        );
        ...
        if (response?.success) {
          ...
          showSuccessAlert(
            "Success",
            response.message ||
              "Work Instruction has been successfully Updated!"
          );

          router.push({ name: "work-instructions" });
          loading.value = false;
        } else {
