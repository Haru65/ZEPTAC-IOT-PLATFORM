<template>
    <div
      class="modal fade"
      tabindex="-1"
      id="kt_mrm_scheudle_export_modal"
        ...
        <div class="modal-content">
          <VForm class="form" novalidate>
            <div class="modal-header">
              <h3 class="modal-title">Export Management Review Meetings Data</h3>
  ...
  import { ExportMrmScheduleData } from "@/stores/api";
  
  export default defineComponent({
    name: "mrm-schedule-export-modal",
    components: {
      ErrorMessage,
      Field,
      VForm,
    ...
    setup(props, { emit }) {
      ...
      const onsubmit = async (exportType: "pdf" | "excel") => {
        ...
        try {
          ...
          // Make the API call using the custom ExportMrmScheduleData function
          const response = await ExportMrmScheduleData(data); // This will await the response from the server
          ...
          const fileName = `mrm_scheudles_${moment().format(
            "YYYYMMDD_HHmmss"
          )}.${fileType}`;
