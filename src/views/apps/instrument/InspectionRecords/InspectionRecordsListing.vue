  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      <Datatable
        ...
        :loading="loading"
      >
        <template v-slot:id="{ row: inspection_instrument }">
          {{ inspection_instrument.id }}
        </template>
        <template v-slot:name="{ row: inspection_instrument }">
          {{ inspection_instrument.name }}
        </template>
        <template v-slot:make="{ row: inspection_instrument }">
          {{ inspection_instrument.make }}
        </template>
        <template v-slot:model_no="{ row: inspection_instrument }">
          {{ inspection_instrument.model_no }}
        </template>
...
import type { IInstrument } from "@/core/model/inspection_instrument";

import { getInspectionRecords, InspectionRecordSearch } from "@/stores/api";
...
export default defineComponent({
  ...
  setup() {
    ...
    async function SearchMore() {
      ...
      try {
        const response = await InspectionRecordSearch(search.value);
