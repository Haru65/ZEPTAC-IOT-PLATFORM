...
const PressureCalibrationGen = async (
    id: any,
    pdfName: string,
    ...
    async function generateUncertainityCalculation(doc, readings: PressureReading[]) {
    ...
    // uuc_readings data
    const UUCData = instrumentDetails.value.pressure_readings && instrumentDetails.value.pressure_readings.map((item, index) => [
      { title: `${index+1}` },
    ...
    const UncertaintyCalculationData = instrumentDetails.value.pressure_readings?.map((item, index) => [
      { title: `${index + 1}` },
      { title: `${instrumentDetails.value.reference_instrument.ranges_from || ""} to ${instrumentDetails.value.reference_instrument.ranges_to || ""}` },
      { title: item.uuc_reading || "" },
    ...
    await generateUncertainityCalculation(doc, instrumentDetails.value.pressure_readings);
